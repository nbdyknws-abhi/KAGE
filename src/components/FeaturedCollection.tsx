        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {PRODUCTS.map((prod) => (
            ...
            >
              ...
              <div 
                className="relative aspect-square overflow-hidden bg-[#0A0A0A] flex items-center justify-center border-b border-white/5"
              >
                <img
                  src={prod.image}
                  alt={prod.name}
                ...
                {/* Glow Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-brand-darker/90 via-brand-darker/20 to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />

                {/* Quick View Button overlay */}
                <div className="absolute bottom-4 left-4 right-4 flex space-x-2 translate-y-0 opacity-100 md:translate-y-8 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 transition-all duration-300">
                  <button
                    onClick={() => setActiveProduct(prod)}
                    className="flex-1 py-3 bg-white text-black md:hover:bg-brand-red md:hover:text-white transition-colors duration-300 font-mono text-[10px] tracking-wider uppercase flex items-center justify-center space-x-1.5"
                  >
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-white/5">
                  ...
                  <button 
                    ...
                    className="p-2 border border-white/10 hover:border-brand-red hover:bg-brand-red/10 text-brand-silver hover:text-white transition-all duration-300"
        ...
        {activeProduct && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto glass border border-white/10 rounded-none text-left grid grid-cols-1 md:grid-cols-2 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Absolute Close Button for Mobile Accessibility */}
              <button
                onClick={() => setActiveProduct(null)}
                className="absolute top-4 right-4 z-10 p-2 bg-brand-dark/80 backdrop-blur-md border border-white/20 text-white/70 hover:text-white hover:bg-brand-red transition-all duration-300 shadow-xl"
              >
                <X className="w-5 h-5" />
              </button>
              {/* Image Side */}
